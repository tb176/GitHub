...
<form action  = "drawCase.do" method = "post" id = "drawCashFrm">
	...
	<!-- 用户余额大于3元是显示blue，小于3元则显示red -->
	<c:choose>
	   <c:when test="${user.balance >= 3}">  
	        <p class="wrong-message" id = "Amount_error" style="color: blue;">少于50元收取2元手续费</p>   		    
	   </c:when>
	   <c:otherwise> 
			<p class="wrong-message" id = "Amount_error">用户最小提现金额是3元</p> 
	   </c:otherwise>
	</c:choose>	
	...
	<ul class="identify-information">
	    ...
	    <li>
	    	<span>手机号</span>
	    	<input name="mobile" id = "mobile" type="text" style="width: 70%;" maxlength = "11" placeholder="请输入银行预留手机号" <c:if test = "${not empty  bankCardInfo.cardMobile}"> value = "${bankCardInfo.cardMobile }" </c:if> />
	    </li>
	</ul>
...
<script type="text/javascript">
	...
	$("#amount").change(function(){
        ...
        $("#Amount_error").removeAttr("style").attr("style","color: blue;").text("少于50元收取2元手续费");
